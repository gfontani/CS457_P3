...
bool update_tentative(vector<int> * temp, vector<vector<int>> * tentative){
	for(int i=0;i<(int)tentative->size(); i++){
		if((*tentative)[i][0] == (*temp)[0] && (*tentative)[i][1] <= (*temp)[1])
			return false;
	}
...
int inspect(int index, vector<vector<int>> * confirmed, vector<vector<int>> * tentative){
	...
	for(int i=0; i<(int)allNeighborWeights[router_to_inspect].size(); i++){
		if((int)allNeighborWeights[router_to_inspect][i]>=0){
				vector<int> temp;
				temp.push_back(i);
...
int ospf(int myRouterNum){
	...
	while(tentative_list.size()>0){
		...
		inspect(index, &confirmed_list, &tentative_list);
	}
